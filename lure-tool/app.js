...
(function() {
  ...
  // Build lookup maps: type→sizes and size→types from actual data
  function buildCombinationMaps() {
    const typeToSizes = {};
    const sizeToTypes = {};
    STATE.luresData.lures.forEach(l => {
      if (!typeToSizes[l.type]) typeToSizes[l.type] = new Set();
      if (!sizeToTypes[l.size_inches]) sizeToTypes[l.size_inches] = new Set();
      typeToSizes[l.type].add(l.size_inches);
      sizeToTypes[l.size_inches].add(l.type);
    });
    // Convert sets to sorted arrays
    Object.keys(typeToSizes).forEach(k => typeToSizes[k] = [...typeToSizes[k]].sort((a,b)=>a-b));
    Object.keys(sizeToTypes).forEach(k => sizeToTypes[k] = [...sizeToTypes[k]].sort());
    return { typeToSizes, sizeToTypes };
  }

  function renderConfiguratorForm() {
    const form = $('#configurator-form');
    const inputs = STATE.rules.user_inputs;
    const { typeToSizes, sizeToTypes } = buildCombinationMaps();

    // All types and sizes from data
    const allTypes = Object.keys(typeToSizes).sort();
    const allSizes = Object.keys(sizeToTypes).map(Number).sort((a,b)=>a-b);

    form.innerHTML = '';

    // Helper: build a select field
    const makeSelect = (id, label, required, placeholder) => {
      const field = el('div', { class: 'form-field' });
      field.appendChild(el('label', {
        class: 'form-label' + (required ? ' form-label--required' : ''),
        text: label
      }));
      const select = el('select', { class: 'form-control', id });
    ...
    // --- 1. Тип приманки (first) ---
    const typeSelect = makeSelect('field-lure_type', 'Тип приманки', true, '— выбери тип —');
    allTypes.forEach(t => typeSelect.appendChild(el('option', { value: t, text: t })));

    // --- 2. Размер приманки (second, filtered) ---
    const sizeSelect = makeSelect('field-size_inches', 'Размер приманки', true, '— выбери размер —');
    const populateSizes = (availableSizes) => {
      const current = sizeSelect.value;
      sizeSelect.innerHTML = '';
      sizeSelect.appendChild(el('option', { value: '', text: '— выбери размер —' }));
      availableSizes.forEach(s => {
        const opt = el('option', { value: String(s), text: s + '″' });
        if (String(s) === current) opt.selected = true;
        sizeSelect.appendChild(opt);
      });
      // If previously selected size no longer valid, clear it
      if (!availableSizes.map(String).includes(current)) {
    ...
    // --- 3. Остальные поля (порядок без size_inches и lure_type) ---
    const order = [
      'target_fish', 'season', 'depth', 'current',
    ...
    order.forEach(key => {
      ...
      const field = el('div', { class: 'form-field' });
      field.appendChild(el('label', { class: 'form-label', text: cfg.label }));

      if (cfg.type === 'single_select') {
        const select = el('select', {
          class: 'form-control',
          id: 'field-' + key,
          onChange: (e) => { STATE.formValues[key] = e.target.value || null; }
        });
        ...
        cfg.options.forEach(opt => {
          const chip = el('button', {
            class: 'chip', type: 'button', text: opt,
            onClick: () => {
  ...
  function handleConfigSubmit() {
    ...
    // Validation
    if (!v.lure_type || !v.size_inches) {
      showResultError('Выбери тип и размер приманки.');
      return;
    }
